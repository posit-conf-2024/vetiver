...
housing |>
  ggplot(aes(long, lat, z = price)) +
  stat_summary_hex(alpha = 0.7) +
  scale_fill_viridis_c(labels = scales::dollar) +
  labs(fill = "Mean price", x = NULL)
```
...
```{r}
library(vetiver)
v <- vetiver_model(housing_fit, "housing-result-rstats")
v
```
...
v = VetiverModel(housing_fit, "housing-result-python", prototype_data = X_train)
v.description
```
...
board <- board_connect()
v <- vetiver_model(housing_fit, "julia.silge/housing-result-rstats")
board |> vetiver_pin_write(v)
```
...
v = VetiverModel(housing_fit, "isabel.zimmerman/housing-result-python", prototype_data = X_train)
vetiver_pin_write(board, v)
```
